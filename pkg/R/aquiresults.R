...
geiger$MLE <- round(c(max(as.numeric(geiger[[1]][,2])),max(as.numeric(geiger[[2]][,2])),max(as.numeric(geiger[[3]][,2])),max(as.numeric(geiger[[4]][,2])),max(as.numeric(geiger[[5]][,2])),max(as.numeric(geiger[[6]][,2])),max(as.numeric(geiger[[7]][,2])),max(as.numeric(geiger[[8]][,2])),max(as.numeric(geiger[[9]][,2])),max(as.numeric(geiger[[10]][,2])),max(as.numeric(geiger[[11]][,2])),max(as.numeric(geiger[[12]][,2]))),5)
...
for (i in c(1:length(well))){
  prptn <- rep(NA,length(geiger[[i]][,1]))
    for (j in c(1:length(prptn))){if (is.na(geiger[[i]][,2][j])){prptn[j] <- 0}else{if (round(as.numeric(geiger[[i]][,2]),4)[j]==tglik){prptn[j] <- 1}else{prptn[j] <- 0}}}
  geiger[[i]] <- cbind(geiger[[i]],prptn)
}
gprptn <- c(round(tglik,3),round(mean(geiger[[1]][,10]),3),round(mean(geiger[[2]][,10]),3),round(mean(geiger[[3]][,10]),3),round(mean(geiger[[4]][,10]),3),round(mean(geiger[[5]][,10]),3),round(mean(geiger[[6]][,10]),3),round(mean(geiger[[7]][,10]),3),round(mean(geiger[[8]][,10]),3),round(mean(geiger[[9]][,10]),3),round(mean(geiger[[10]][,10]),3),round(mean(geiger[[11]][,10]),3),round(mean(geiger[[12]][,10]),3))
...
png(file="geiger_aqui_propvstime.png")
plotCI(time.tab[,1],time.tab[,2],ui=UI,li=LI,xlim=c(min(MIN),max(MAX)),err="x",xlab="Time (Seconds)",ylab="Proportion",main="fitContinuous Aquilegia",pch=NA,gap=3)
for (i in c(1:length(well)))
  {
...
png(file="ace_aqui_propvstime.png")
plotCI(time.tab[,1],time.tab[,2],ui=UI,li=LI,xlim=c(min(MIN),max(MAX)),err="x",xlab="Time (Seconds)",ylab="Proportion",main="Ace Aquilegia",pch=NA,gap=3)
for (i in c(1:length(well)))
  {
  text(x=time.tab[,1][i],y=time.tab[,2][i],labels=well[i])
}
...
png(file="ouch_aqui_propvstime.png")
plotCI(time.tab[,1],time.tab[,2],ui=UI,li=LI,xlim=c(min(MIN),max(MAX)),err="x",xlab="Time (Seconds)",ylab="Proportion",main="Ouch Aquilegia",pch=NA,gap=3)
for (i in c(1:length(well)))
  {
  text(x=time.tab[,1][i],y=time.tab[,2][i],labels=well[i])
}
