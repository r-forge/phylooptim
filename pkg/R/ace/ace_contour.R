    ...
    for (i in c(1:length(well))){
     ...
     filled.contour(x1range,x2range,res,levels=levels,col=rcols,  
      		plot.axes={
      			axis(1)
      			axis(2)
            plot.title=title(main="title",xlab="sigma2", ylab="ace")
   #   			contour(x1range,x2range,res,add=TRUE,nlevels=length(levels))
                        
     	                arrows(xxsig[[i]][s[[i]]], yyace[[i]][s[[i]]], xxsig[[i]][s[[i]]+1],
                              yyace[[i]][s[[i]]+1],length=.1,col="yellow",
                              type = "l",lwd=2)
                       points(l[[i]]$sigma2,l[[i]]$ace[1],col=2,pch=15,cex=3)
                       points(xx[[i]][1],yy[[i]][1],col="green",pch=19,cex=2)
                        
      		}

  
      png(file="contourall.png")
      for (i in c(1:length(well))){
        i <- 1
      par(mfrow = c(1,1))
      filled.contour(x1range,x2range,res,levels=levels,col=rcols,  
