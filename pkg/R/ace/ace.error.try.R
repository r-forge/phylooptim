...
for (i in c(1:length(well))){
  if (well[i]==wellt[i]){
        ...
        out <- tryCatch(optimx(function(p) dev.BM(p), p=c(1,rep(mean(x), length.out = nb.node)),
                      method=well[i],hessian=TRUE), error=function(x){x <- NA
                                                         return(x)})
	lb <- NA;ub <- NA;time <- as.numeric(proc.time()[3]-begin.time[3])/(60)}
if (is.na(out[1])){obj$conv <- NA
                obj$time <- time
                obj$lb <- lb
                obj$ub <- ub
                obj$loglik <- NA
                obj$ace <- NA
                se <- NA
                obj$sigma2 <- c(NA)}else{
                obj$conv <- out$conv$conv
                obj$time <- time
                obj$lb <- lb
                obj$ub <- ub
                obj$loglik <- -out$fvalues$fvalues/2   
                obj$ace <- out$par$par[-1]
                names(obj$ace) <- (nb.tip + 1):(nb.tip + nb.node)
...
#For Continuous
begin.time <- proc.time()
l1 <- ace(as.vector(dv$data),dv$phy,type='continuous',ip=1,CI=FALSE)

#Time in minutes
total.time <- as.numeric(proc.time()[3]-begin.time[3])/(60)

#Number of starting points (CONTINUOUS DATA)
#m<- 5
...
save.image("/home/michels/repository/phylooptim/pkg/R/ace/tryaceerror.RData")
