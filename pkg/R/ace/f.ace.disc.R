...
names(lt) <- well
 ...
 k<-rbind(k,apply(k,2,function(x) {ace(dv$data,dv$phy,type='discrete',ip=x,CI=FALSE)}))
            ...
            for(j in c(1:length(optimx))){
              if (well[j]==wellt[j]){
                lt[[j]]<-as.data.frame(t(rbind(unlist(k[1,]),as.data.frame(matrix(unlist(lapply(optimx[[j]],function(x) return(c(x$lb,x$ub,x$rates,x$loglik,x$conv,x$time)))),6,ncol(k))))))
               colnames(lt[[j]])<-c("I","lb","ub","P","L","conv","time")}else{
                lt[[j]]<-as.data.frame(t(rbind(unlist(k[1,]),as.data.frame(matrix(unlist(lapply(optimx[[j]],function(x) return(c(NA,NA,x$rates,x$loglik,x$conv,x$time)))),6,ncol(k))))))                  
         colnames(lt[[j]])<-c("I","lb","ub","P","L","conv","time")}
}
return(lt)
}
