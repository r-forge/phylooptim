#Make sure to have acebackupdiscrete.RData in the correct directory
rm(list = ls())
require(ouch)
require(optimx)
require(geiger)
...
names(l) <- well
for (i in c(1:length(well))){l[[i]] <- matrix(NA,ncol=7,nrow=it);colnames(l[[i]]) <- c("I","lb","ub","P","L","conv","time")}
...
save.image(paste(getwd(),"/",name,"aceerrordisc.RData",sep=""))
