...
hansen.new <- function (data, tree, regimes, sqrt.alpha, sigma, itn,
                    fit = TRUE,
...
par<-c(sqrt.alpha,sigma)
  ...
  for (i in c(1:length(out))){
	...
	llhstore[[i]]=list(sqrt.alpha=sqrt.alpha,sigma=sigma)
	out[[i]][[1]] <- tryCatch(optimx(
                     ...
                     ou.lik.fn(
                      ...
                      method=well[i],itnmax=1
                      ), error=function(x){
                                           x$par[seq(nalpha)] <- NA
                                           x$par[nalpha+seq(nsigma)] <- NA
                                           return(x)})

  llhstore[[i]]$sqrt.alpha <- c(llhstore[[i]]$sqrt.alpha,as.numeric(unlist(out[[i]][[1]]$par)[1]))
  llhstore[[i]]$sigma <- c(llhstore[[i]]$sigma,as.numeric(unlist(out[[i]][[1]]$par)[2]))
#  if (is.na(unlist(out[[i]][[1]]$par)[seq(nalpha)])){llhstore[[i]]$loglik <- c(llhstore[[i]]$loglik,NA)}else{
#  llhstore[[i]]$loglik <- c(llhstore[[i]]$loglik,-0.5*ou.lik.fn(
#                   tree=tree,
#                   alpha=sym.par(unlist(out[[i]][[1]]$par)[seq(nalpha)]),
#                   sigma=sym.par(unlist(out[[i]][[1]]$par)[nalpha+seq(nsigma)]),
#                   beta=beta,
#                   dat=dat
#                   )$deviance)}
...
for (j in c(2:(itn-1))){
	out[[i]][[1]] <- tryCatch(optimx(
                     ...
                     p=c(sqrt.alpha,sigma),
                     lower = rep(0.001,(nalpha+nsigma)), upper = rep(1e50,(nalpha+nsigma)),
                     method=well[i],itnmax=j
                     ), error=function(x){
                                          x$par[seq(nalpha)] <- NA
                                          x$par[nalpha+seq(nsigma)] <- NA
                                          return(x)})

  llhstore[[i]]$sqrt.alpha <- c(llhstore[[i]]$sqrt.alpha,as.numeric(unlist(out[[i]][[1]]$par)[seq(nalpha)]))
  llhstore[[i]]$sigma <- c(llhstore[[i]]$sigma,as.numeric(unlist(out[[i]][[1]]$par)[nalpha+seq(nsigma)]))
#  if (is.na(unlist(out[[i]][[1]]$par)[seq(nalpha)])){llhstore[[i]]$loglik <- c(llhstore[[i]]$loglik,NA)}else{
#  llhstore[[i]]$loglik <- c(llhstore[[i]]$loglik,-0.5*ou.lik.fn(
#                   tree=tree,
#                   alpha=sym.par(unlist(out[[i]][[1]]$par)[seq(nalpha)]),
...
well <- c("spg", "Rcgmin", "Rvmmin", "bobyqa","L-BFGS-B","nlminb","ucminf","Nelder-Mead","nlm","CG","BFGS","newuoa")
  ...
  for (i in c(1:length(obj.list))){obj.list[[i]] <- vector("list", 4)}
  names(obj.list) <- well
...
#     se.sigma <- rep(NA,nalpha)
  ...
  if (is.na(sqrt.alpha)){sol$coeff <- NaN;sol$deviance <- NaN}else{
  sol <- ou.lik.fn(
                   tree=tree,
  ...
  if (is.na(sqrt.alpha)){sqrt.alpha=NaN}
  if (is.na(sigma)){sigma=NaN}
  ...
  obj.list[[i]][[2]] <- llhstore[[i]]
  obj.list[[i]][[3]] <- time
  obj.list[[i]][[4]] <- out[[i]][[1]]$conv$conv
  }
obj.list
}
