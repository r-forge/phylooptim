...
ouch.wrap.x<-function(tree, trait, model=c("brown", "ou1", "ousm"),y){
	...
	if (is.character(model)) {
		...
		if (model == "ou1"){
			...
			obj <- hansen(tree=ot,data=otd["phenoTrait"],regimes=otd["regimes"],sqrt.alpha=s,sigma=s, maxit=10000)
		}
		...
		if (model == "ousm"){
			...
			obj <- hansen(tree=ot,data=otd["phenoTrait"],regimes=otd["regime"],sqrt.alpha=s,sigma=s, maxit=10000)
		}
