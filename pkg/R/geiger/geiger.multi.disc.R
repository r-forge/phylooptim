#Make sure to have geigerbackupdisc.RData in the correct directory
rm(list = ls())
require(ouch)
require(optimx)
require(geiger)
...
names(l) <- well
for (i in c(1:length(well))){l[[i]] <- matrix(NA,ncol=9,nrow=it);colnames(l[[i]]) <- c("lb1","ub1","S1","lb2","ub2","S2","L","time","conv")}
