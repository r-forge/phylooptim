`fitDiscrete` <-
function(phy, data, model=c("ER", "SYM", "ARD"), treeTransform=c("none", "lambda", "kappa", "delta", "linearChange", "exponentialChange", "twoRate"), data.names=NULL, plotlnl=F, qLimits=c(0.0001, 1000), pLimits=c(0.00001, 10),s=c(-5))
{
...
if (well[k]==wellt[k]){
			...
			sp<-c(pStart, log(rep(qTries, nRateCats)))
				te<-tryCatch(optimx(f, p=sp,
                                             lower=lower,upper=upper,method=well[k]),
                                             error=function(x){x <- NA
                                                               return(x)})
                lb <- lower;ub <- upper;start <- sp;
                time <- as.numeric(proc.time()[3]-begin.time[3])/(60)}else{
                begin.time <-proc.time()
                          	sp<-c(pStart, log(rep(qTries, nRateCats)))
				te<-tryCatch(optimx(f, p=sp, method=well[k]),
                                             error=function(x){x <- NA
                                                               return(x)})
                lb <- c(NA,NA);ub <- c(NA,NA);start <- sp;
...
#te$par == k$par$par
#te$value == k$fvalues$fvalues
			if(treeTransform=="none" & model=="ER") {
				res[[i]]<-list(lnl=-out$fvalues$fvalues, q=-getQ(exp(out$par$par), nRateCats, model)[1,1],lb=lb,ub=ub,start=start, conv=out$conv$conv, time=time)
			} else if(treeTransform=="none" & model!="ER") {
				res[[i]]<-list(lnl=-out$fvalues$fvalues, q=getQ(exp(out$par$par), nRateCats, model),lb=lb,ub=ub,start=start, conv=out$conv$conv, time=time)

			} else if(treeTransform=="twoRate") {
				res[[i]]<-list(lnl=-out$fvalues$fvalues, q=getQ(exp(out$par$par[-(1:2)]), nRateCats, model), breakpoint=out$par$par[1], endRate=exp(out$par$par[2]),lb=lb,ub=ub,start=start, conv=out$conv$conv, time=time)
			} else 	res[[i]] <- if (is.na(out[1])){list(lnl=NA,q=NA,treeParam=NA,lb=lb,ub=ub,start=start,conv=NA,time=time)}else{list(lnl=-out$fvalues$fvalues, q=getQ(exp(out$par$par[-1]), nRateCats, model), treeParam=exp(out$par$par[1]), lb=lb, ub=ub, start=start, conv=out$conv$conv, time=time)}
