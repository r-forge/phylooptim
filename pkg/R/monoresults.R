...
geiger$MLE <- round(c(max(as.numeric(geiger[[1]][,1])),max(as.numeric(geiger[[2]][,1])),max(as.numeric(geiger[[3]][,1])),max(as.numeric(geiger[[4]][,1])),max(as.numeric(geiger[[5]][,1])),max(as.numeric(geiger[[6]][,1])),max(as.numeric(geiger[[7]][,1])),max(as.numeric(geiger[[8]][,1])),
#Sidenote, value for nlm -320.0083 is not the likelihood.
                      min(as.numeric(geiger[[9]][,1])),max(as.numeric(geiger[[10]][,1])),max(as.numeric(geiger[[11]][,1])),max(as.numeric(geiger[[12]][,1]))),5)
...
png(file="ace_mono_propvstime.png")
plotCI(time.tab[,1],time.tab[,2],ui=UI,li=LI,xlim=c(min(MIN),max(MAX)),err="x",xlab="Time (Seconds)",ylab="Proportion",main="Ace Monocot",pch=NA,gap=3)
for (i in c(1:length(well)))
  {
  text(x=time.tab[,1][i],y=time.tab[,2][i],labels=well[i])
}
